...
export function ToolbarContextProvider({
  ...
  const focusedItemIdRef = React.useRef(null);
  const [items, setItems] = React.useState([]);
  const updateFocusableItemId = React.useCallback(id => {
  ...
  const onItemBlur = React.useCallback((id, event) => {
  ...
  React.useEffect(() => {
    ...
    if (!focusableItemExists) {
      ...
      if (focusedItemIdRef.current !== null && !sortedItems.some(item => item.id === focusedItemIdRef.current)) {
  ...
  const contextValue = React.useMemo(() => ({
    ...
    onItemBlur,
    onItemDisabled
  }), [focusableItemId, registerItem, unregisterItem, onItemKeyDown, onItemFocus, onItemBlur, onItemDisabled]);
