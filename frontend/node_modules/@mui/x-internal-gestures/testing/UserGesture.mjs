...
export class UserGesture {
  ...
  setup(options) {
    ...
    options?.plugins?.forEach(plugin => {
      ...
      if (this[plugin.name]) {
        throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: Plugin with name "${plugin.name}" already exists. Please use a unique name.` : _formatErrorMessage(169, plugin.name));
      }
      // @ts-expect-error, we are using a dynamic key
