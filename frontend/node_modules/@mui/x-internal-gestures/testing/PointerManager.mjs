...
export class PointerManager {
  ...
  updatePointers(pointer) {
    ...
    if (!existingPointer) {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: Pointer with id ${pointer.id} does not exist` : _formatErrorMessage(166, pointer.id));
    }
  ...
  parseMousePointer(pointer, target) {
    if (this.mode !== 'mouse') {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X: Mouse pointer can only be used in mouse mode' : _formatErrorMessage(167));
    }
  ...
  parsePointers(pointers, target, defaultConfig) {
    ...
    if (this.mode === 'mouse') {
      ...
      if (Array.isArray(normalizedPointers) && normalizedPointers.length > 1 || !Array.isArray(normalizedPointers) && normalizedPointers.amount !== 1) {
        throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X: Mouse mode only supports one pointer' : _formatErrorMessage(168));
      }
    }
