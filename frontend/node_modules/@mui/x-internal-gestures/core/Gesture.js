...
class Gesture {
  ...
  constructor(options) {
    if (!options || !options.name) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X: Gesture must be initialized with a valid name.' : (0, _formatErrorMessage2.default)(164));
    }
    if (options.name in _eventList.eventList) {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: Gesture can't be created with a native event name. Tried to use "${options.name}". Please use a custom name instead.` : (0, _formatErrorMessage2.default)(165, options.name));
    }
